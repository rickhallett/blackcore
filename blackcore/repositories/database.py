...
class DatabaseRepository(BaseRepository[NotionDatabase]):
    ...
    def update(self, id: str, data: Dict[str, Any]) -> NotionDatabase:
        ...
        with self._operation_context("update", id):
            try:
                ...
                # Log data access
                self._log_data_access("update", id, fields=list(update_data.keys()))
    ...
    def query(
        self, database_id: str, query: Optional[NotionDatabaseQuery] = None, **kwargs
    ) -> List[NotionPage]:
        ...
        with self._operation_context("query", database_id):
            try:
                ...
                # Log data access
                self._log_data_access("query", database_id, fields=["pages"])
    ...
    def add_property(
        self, database_id: str, property_name: str, property_config: Dict[str, Any]
    ) -> NotionDatabase:
    ...
    def _format_title(self, title: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(title, str):
            ...
            return [{"type": "text", "text": {"content": str(title)}}]
        ...
        for name, config in properties.items():
            if config is None:
                ...
                if "type" not in config and name not in config:
                    raise RepositoryError(f"Property '{name}' must have 'type' field")
                formatted[name] = config
            else:
                raise RepositoryError(f"Property '{name}' config must be dict or None")
