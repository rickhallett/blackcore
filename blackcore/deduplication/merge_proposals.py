...
class MergeExecutor:
        ...
        for field in identifier_fields:
            ...
            if val_a_set and val_b_set:
                if val_a_set.isdisjoint(val_b_set):
                    ...
                    logger.warning(
                        f"Identifier conflict in {field}: {val_a_set} vs {val_b_set}"
                    )
                    return True
    ...
    def _has_significant_data_disparity(self, proposal: MergeProposal) -> bool:
        ...
        ratio = max(primary_fields, secondary_fields) / min(
    ...
    def _identify_risk_factors(self, proposal: MergeProposal) -> List[str]:
        ...
        if (
            ...
            and proposal.ai_analysis.get("recommended_action") == "needs_human_review"
        ):
        ...
        if (
            proposal.ai_analysis
            and proposal.ai_analysis.get("risk_assessment") == "high"
        ):
    ...
    def _determine_merge_strategy(self, proposal: MergeProposal) -> str:
        ...
        if (
            proposal.confidence_score >= 95
            and not proposal.risk_factors
            and not proposal.safety_checks
        ):
            return "aggressive"
    ...
    def execute_merge(
        self, proposal: MergeProposal, auto_approved: bool = False
    ) -> MergeResult:
        ...
        try:
            ...
            if validation_errors:
                return MergeResult(success=False, errors=validation_errors)
            ...
            return MergeResult(
                success=True, merged_entity=merged_entity, rollback_info=backup_info
            )
    ...
    def _is_merge_allowed(self, proposal: MergeProposal, auto_approved: bool) -> bool:
        ...
        if self.config.get("enable_safety_checks", True) and proposal.safety_checks:
            logger.warning(
                f"Safety checks failed for {proposal.proposal_id}: {proposal.safety_checks}"
            )
            return False
        ...
        if not auto_approved and proposal.confidence_score < self.config.get(
            "auto_approve_threshold", 95.0
        ):
            return False
    ...
    def _validate_merged_entity(self, merged_entity: Dict[str, Any]) -> List[str]:
        ...
        if not merged_entity.get("Full Name") and not merged_entity.get(
            "Organization Name"
        ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        success_rate = (
            self.stats["successful_merges"] / max(total_operations, 1)
        ) * 100
