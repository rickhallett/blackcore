...
class DeduplicationEngine:
    ...
    def analyze_database(
        self, database_name: str, records: List[Dict[str, Any]], enable_ai: bool = True
    ) -> DeduplicationResult:
        """
        ...
        processor = self.processors.get(
            database_name, self.processors["Documents & Evidence"]
        )

        # Generate candidate pairs for comparison
        candidate_pairs = self._generate_candidate_pairs(records, processor)
        logger.info(
            f"   🔍 Generated {len(candidate_pairs)} candidate pairs for analysis"
        )
        ...
        result.confidence_distribution = self._calculate_confidence_distribution(
            candidate_pairs
        )

        # Execute automatic merges if in production mode
        if not self.config["safety_mode"]:
            result.auto_merged = self._execute_automatic_merges(
        ...
        logger.info(
            f"   ❓ Low confidence matches: {len(result.low_confidence_matches)}"
        )

        return result

    def _generate_candidate_pairs(
        self, records: List[Dict[str, Any]], processor
    ) -> List[EntityPair]:
    ...
    def _analyze_entity_pair(self, pair: EntityPair, processor, enable_ai: bool):
        ...
        pair.confidence_score = processor.calculate_confidence(
            pair.similarity_scores, pair.entity_a, pair.entity_b
        )

        # Layer 2: AI/LLM analysis for medium+ confidence matches
        if (
            enable_ai
            and pair.confidence_score >= self.config["human_review_threshold"]
            and self.config["enable_ai_analysis"]
        ...
        pair.human_review_required = (
            ...
            or (
                pair.ai_analysis
                and pair.ai_analysis.recommended_action == "needs_human_review"
            )
        )
    ...
    def _execute_automatic_merges(
        self, high_confidence_matches: List[Dict[str, Any]]
    ) -> int:
    ...
    def _create_review_tasks(
        self, medium_confidence_matches: List[Dict[str, Any]], database_name: str
    ) -> int:
    ...
    def analyze_all_databases(
        ...
        total_potential_duplicates = sum(
            result.potential_duplicates for result in results.values()
        )
        ...
        logger.info(
            f"   🔍 Total potential duplicates found: {total_potential_duplicates}"
        )
        logger.info(
            f"   🤖 AI analyses performed: {self.stats['ai_analyses_performed']}"
        )
        logger.info(
            f"   📋 Human reviews created: {self.stats['human_reviews_created']}"
        )
