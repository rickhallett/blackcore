...
class ConfigurationWizard:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "thresholds": {"auto_merge": 90.0, "human_review": 70.0},
            "ai": {
                "enabled": True,
            ...
            "ui": {"page_size": 10, "show_memory": False, "color_scheme": "default"},
        }

    async def run_wizard(
        self, available_databases: Optional[Dict[str, int]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def _configure_thresholds(self) -> Dict[str, float]:
        ...
        return {"auto_merge": auto_merge, "human_review": review}

    def _prompt_threshold(
        self, prompt: str, default: float, min_value: float, max_value: float
    ) -> float:
        """Prompt for a threshold value with validation."""
        while True:
            try:
                value = FloatPrompt.ask(prompt, default=default, show_default=True)
    ...
    async def _configure_ai(self) -> Dict[str, Any]:
        ...
        enable_ai = Confirm.ask(
            "[yellow]Enable AI-powered analysis?[/yellow]", default=True
        )
        ...
        max_concurrent = IntPrompt.ask(
            "[yellow]Max concurrent AI requests[/yellow]", default=5, show_default=True
        )
    ...
    async def _configure_processing(self) -> Dict[str, Any]:
        ...
        batch_size = IntPrompt.ask(
            "[yellow]Batch size for processing[/yellow]", default=100, show_default=True
        )

        enable_graph = Confirm.ask(
            "[yellow]Enable graph-based relationship analysis?[/yellow]", default=True
        )

        safety_mode = Confirm.ask(
            "[yellow]Enable safety mode (no automatic changes)?[/yellow]", default=True
        )
    ...
    async def _review_configuration(self) -> bool:
        ...
        if "databases" in self.config:
            summary_lines.append(
                f"[bold]Databases:[/bold] {', '.join(self.config['databases'])}"
            )
