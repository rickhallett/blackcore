...
Press [bold]Enter[/bold] to continue..."""
    ...
    def create_main_menu(self) -> Panel:
        ...
        return Panel(content, title="Main Menu", border_style="blue", padding=(1, 2))
    ...
    def create_threshold_config_panel(
        self, auto_merge: float = 90.0, review: float = 70.0
    ) -> Panel:
...
[bold]Impact Preview:[/bold]
  ...
  • Low confidence (<{review}%): Likely not duplicates"""
    ...
    def prompt_threshold(
        self, prompt_text: str, default: int, min_value: int = 0, max_value: int = 100
    ) -> int:
        """Prompt for threshold value with validation."""
        while True:
            try:
                value = IntPrompt.ask(prompt_text, default=default, show_default=True)
...
class ProgressTracker:
    ...
    def create_dashboard_layout(self) -> Layout:
        ...
        layout["footer"].update(
            Panel("[dim]Press Ctrl+C to cancel[/dim]", border_style="dim")
        )
    ...
    async def update_progress(self, update: ProgressUpdate):
        ...
        if hasattr(self, "current_task"):
            ...
            self.current_task = self.progress.add_task(
                update.stage, total=update.total, completed=update.current
            )
    ...
    def _update_stats(self, update: ProgressUpdate):
        ...
        stats_lines = [
            ...
            (
                f"[bold]Percentage:[/bold] {(update.current/update.total*100):.1f}%"
                if update.total > 0
                else "0%"
            ),
        ]
        ...
        self.stats_panel = Panel(
            "\n".join(stats_lines), title="Statistics", border_style="blue"
        )
...
class MatchReviewDisplay:
    ...
    def create_match_comparison(
        self, match: Dict[str, Any], match_number: int, total_matches: int
    ) -> Layout:
        ...
        comparison_table = self._create_comparison_table(
            match.get("entity_a", {}), match.get("entity_b", {}), primary_entity
        )
    ...
    def _create_comparison_table(
        ...
        table = Table(
            box=box.SIMPLE_HEAD, show_header=True, header_style="bold", expand=True
        )
        ...
        display_fields = [f for f in all_fields if not f.startswith("_") and f != "id"]
    ...
    def _format_evidence(self, match: Dict[str, Any]) -> str:
        ...
        return (
            "\n".join(evidence_lines)
            if evidence_lines
            else "[dim]No additional evidence[/dim]"
        )
    ...
    def display_review_summary(
        self, total_reviewed: int, approved: int, rejected: int, deferred: int
    ) -> Panel:
...
[dim]Results have been saved to the audit system.[/dim]"""
        ...
        return Panel(summary, title="Summary", border_style="cyan", padding=(1, 2))
...
[dim]Press any key to continue...[/dim]"""
        ...
        return Panel(help_text, title="Help", border_style="blue", padding=(1, 2))
    ...
    def create_merge_preview(
        ...
        preview_lines.append(
            f"Fields filled from secondary: [green]{len(filled_fields)}[/green]"
        )
