...
class StandardModeCLI:
    ...
    async def _new_analysis(self):
        ...
        self.console.print(
            Panel(
                "[bold cyan]New Deduplication Analysis[/bold cyan]", border_style="cyan"
            )
        )
        ...
        if json_dir.exists():
            for json_file in json_dir.glob("*.json"):
                ...
                try:
                    ...
                    self.console.print(
                        f"[yellow]Warning: Could not load {json_file}: {e}[/yellow]"
                    )
        else:
            self.console.print(
                f"[yellow]Warning: JSON directory not found at {json_dir}[/yellow]"
            )
        ...
        try:
            ...
            async for _ in tracker.track_progress():
                results = await self.engine.analyze_databases_async(
                    databases, progress_callback=on_progress
                )
...
[bold]Total matches available for review:[/bold] {high_confidence + medium_confidence + low_confidence}"""
        ...
        self.console.print(
            Panel(
                summary, title="Analysis Summary", border_style="green", padding=(1, 2)
            )
        )
    ...
    async def _review_matches(self, results: Dict[str, Any]):
        ...
        while current_index < len(matches_to_review):
            ...
            layout = review_display.create_match_comparison(
                match, current_index + 1, len(matches_to_review)
            )
            ...
            if action == "a":  # Approve
                ...
                self._record_decision(
                    match, "separate", "User rejected - not duplicates"
                )
                current_index += 1
            elif action == "d":  # Defer
    ...
    async def _get_review_action(self) -> str:
        ...
        while True:
            action = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: Prompt.ask(
                    "\n[cyan]Action[/cyan]", choices=valid_actions, show_choices=False
                ).lower(),
            )
    ...
    async def _show_merge_preview(
        self, match: Dict[str, Any], review_display: MatchReviewDisplay
    ):
        """Show merge preview."""
        self.console.clear()
    ...
    async def _show_review_summary(self, review_display: MatchReviewDisplay):
        ...
        summary_panel = review_display.display_review_summary(
            len(self.review_decisions), approved, rejected, deferred
        )
    ...
    async def _prompt_apply_decisions(self) -> bool:
        ...
        approved_count = sum(
            1 for d in self.review_decisions if d["decision"] == "merge"
        )
    ...
    async def _apply_review_decisions(self):
        ...
        self.console.print(
            Panel(
                "[bold cyan]Applying Review Decisions[/bold cyan]", border_style="cyan"
            )
        )
        ...
        if (
            self.engine
            and hasattr(self.engine, "engine")
            and hasattr(self.engine.engine, "config")
        ):
...
• Review decisions made: {len(self.review_decisions)}"""
        ...
        self.console.print(
            Panel(stats_text, title="Statistics", border_style="blue", padding=(1, 2))
        )
...
See documentation at: dedupe-engine-commands.md"""
        ...
        self.console.print(
            Panel(help_text, title="Help", border_style="blue", padding=(1, 2))
        )
