...
class HumanReviewInterface:
    ...
    def get_next_review_task(
        self, reviewer_id: str, priority: Optional[str] = None
        ...
        pending_tasks = self.audit_system.get_pending_reviews(
            reviewer_id=None, priority=priority  # Get unassigned tasks
        )
    ...
    def _build_review_context(self, task) -> ReviewContext:
        ...
        context.risk_factors = self._identify_risk_factors(
            entity_a, entity_b, similarity_analysis
        )

        # Extract evidence
        context.supporting_evidence = self._extract_supporting_evidence(
            entity_a, entity_b, similarity_analysis
        )
        context.conflicting_evidence = self._extract_conflicting_evidence(
            entity_a, entity_b
        )
    ...
    def _generate_reviewer_guidance(self, context: ReviewContext) -> Dict[str, Any]:
        ...
        if context.ai_analysis:
            ...
            if ai_confidence >= 90:
                guidance["confidence_indicators"].append(
                    "AI analysis shows high confidence"
                )
            elif ai_confidence <= 50:
                guidance["confidence_indicators"].append(
                    "AI analysis shows low confidence"
                )
        ...
        if len(context.risk_factors) == 0:
            ...
            guidance["red_flags"].append(
                "Multiple risk factors present - proceed with caution"
            )
    ...
    def submit_review_decision(self, decision: ReviewDecision) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.error(
                    f"Failed to submit review decision for task {decision.task_id}"
                )
                return False
    ...
    def _validate_decision(self, decision: ReviewDecision) -> List[str]:
        ...
        if decision.time_spent_seconds > (self.config["max_review_time_minutes"] * 60):
            errors.append(
                f"Review time exceeded maximum of {self.config['max_review_time_minutes']} minutes"
            )
    ...
    def _update_review_statistics(self, decision: ReviewDecision):
        ...
        total_time = self.stats["average_review_time"] * (
            self.stats["reviews_completed"] - 1
        )
        ...
        if (
            decision.time_spent_seconds > 300  # More than 5 minutes
            or decision.confidence < 70
            or "difficult" in decision.flags
        ):
    ...
    def get_reviewer_dashboard(self, reviewer_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_task_priority": (
                assigned_tasks[0].priority if assigned_tasks else None
            ),
            "reviewer_stats": reviewer_stats,
            "system_stats": {
                "total_pending_reviews": len(self.audit_system.get_pending_reviews()),
                "average_review_time": self.stats["average_review_time"],
                "difficult_cases_percentage": (
                    self.stats["difficult_cases"]
                    / max(self.stats["reviews_completed"], 1)
                )
                * 100,
            },
            "guidance": {
                "suggested_daily_reviews": 10,
                "estimated_time_per_review": self.stats["average_review_time"]
                / 60,  # minutes
                "quality_tips": [
    ...
    def generate_review_report(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        if report["performance_metrics"]["average_review_time_minutes"] > 10:
            report["recommendations"].append(
                "Consider providing additional reviewer training to improve efficiency"
            )

        if report["performance_metrics"]["difficult_cases_rate"] > 30:
            report["recommendations"].append(
                "High rate of difficult cases - consider improving AI analysis or pre-screening"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {"interface_stats": self.stats.copy(), "configuration": self.config}
