...
class SimilarityScorer:
    ...
    def calculate_similarity(self, entity_a: Dict[str, Any], entity_b: Dict[str, Any], 
                           comparison_fields: List[str]) -> Dict[str, float]:
        """
    ...
    def _calculate_field_similarity(self, value_a: str, value_b: str, field_name: str) -> Dict[str, float]:
        ...
        scores["difflib"] = difflib.SequenceMatcher(None, value_a, value_b).ratio() * 100
        ...
        if JELLYFISH_AVAILABLE:
            try:
                scores["soundex"] = 100.0 if jellyfish.soundex(value_a) == jellyfish.soundex(value_b) else 0.0
                scores["metaphone"] = 100.0 if jellyfish.metaphone(value_a) == jellyfish.metaphone(value_b) else 0.0
            except:
                scores["soundex"] = 0.0
        ...
        if "name" in field_name.lower():
            ...
            scores["organization_specific"] = self._calculate_organization_similarity(value_a, value_b)
        elif any(loc in field_name.lower() for loc in ["address", "location", "place"]):
            scores["location_specific"] = self._calculate_location_similarity(value_a, value_b)
        else:
    ...
    def _token_sort_similarity(self, a: str, b: str) -> float:
        ...
        return difflib.SequenceMatcher(None, " ".join(tokens_a), " ".join(tokens_b)).ratio() * 100
        ...
        for token_a in tokens_a:
            for token_b in tokens_b:
                ...
                for full_name, nicknames in self.name_patterns["nicknames"].items():
                    if ((token_a == full_name and token_b in nicknames) or
                        (token_b == full_name and token_a in nicknames) or
                        (token_a in nicknames and token_b in nicknames)):
                        return True
    ...
    def _calculate_composite_field_score(self, scores: Dict[str, float], field_name: str) -> float:
    ...
    def _calculate_weighted_score(self, field_scores: Dict[str, Any], comparison_fields: List[str]) -> float:
