...
class LLMEntityAnalyzer:
    ...
    def analyze_entity_pair(
        self, entity_a: Dict[str, Any], entity_b: Dict[str, Any], entity_type: str
        ...
        try:
            ...
            if (
                self.config.get("enable_cross_validation", False)
                and len(self.clients) > 1
            ):
                secondary_result = self._analyze_with_model(
                    prompt, self.config["fallback_model"]
                )
            ...
            logger.debug(
                f"LLM analysis completed in {processing_time:.2f}s with {result.confidence_score:.1f}% confidence"
            )
    ...
    def _generate_person_prompt(
        self, person_a: Dict[str, Any], person_b: Dict[str, Any]
    ) -> str:
...
- Low risk: Strong evidence, minimal conflicts"""
    ...
    def _generate_organization_prompt(
        self, org_a: Dict[str, Any], org_b: Dict[str, Any]
    ) -> str:
...
{{
    ...
    def _extract_action_from_text(self, text: str) -> str:
        ...
        if "merge" in text_lower and "not" not in text_lower:
            ...
            for phrase in ["human review", "manual review", "uncertain"]
        ):
    ...
    def _combine_results(
        self, primary: LLMAnalysisResult, secondary: LLMAnalysisResult
    ) -> LLMAnalysisResult:
