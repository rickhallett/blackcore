...
class GraphRelationshipAnalyzer:
    ...
    def build_relationship_graph(self, databases: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    def _process_database_entities(self, db_name: str, entities: List[Dict[str, Any]]) -> None:
    ...
    def _generate_entity_id(self, entity: Dict[str, Any], db_name: str) -> str:
        ...
        key_fields = ["Full Name", "Organization Name", "Event / Place Name", "Title", "Entry Title"]
    ...
    def _find_relationships(self, node_a: EntityNode, node_b: EntityNode) -> List[RelationshipEdge]:
    ...
    def _check_shared_organization(self, entity_a: Dict[str, Any], entity_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for field in org_fields:
            ...
            if val_a and val_b and val_a == val_b:
                ...
                total_strength += self.config["relationship_weights"]["shared_organization"]
                matches += 1
    ...
    def _check_shared_location(self, entity_a: Dict[str, Any], entity_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        for field in location_fields:
            ...
            if val_a and val_b:
                ...
                if val_a == val_b:
                    ...
                    evidence.append(f"Similar {field}: {entity_a.get(field)} / {entity_b.get(field)}")
                    matches += 0.5
                    
        if matches > 0:
            strength = min(matches * self.config["relationship_weights"]["shared_location"], 1.0)
            return {
                "strength": strength,
                "evidence": evidence,
    ...
    def _check_shared_event(self, entity_a: Dict[str, Any], entity_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _check_contact_relationship(self, entity_a: Dict[str, Any], entity_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_centrality_scores(self) -> None:
        ...
        max_connections = max(len(node.connections) for node in self.nodes.values()) if self.nodes else 1
        
        for node in self.nodes.values():
            # Degree centrality normalized by maximum possible connections
            node.centrality_score = len(node.connections) / max_connections if max_connections > 0 else 0.0
            
    def analyze_for_disambiguation(self, entity_pairs: List[Tuple[str, str]]) -> GraphAnalysisResult:
        """Analyze entity pairs using graph-based methods for disambiguation."""
        
        logger.info(f"🔍 Analyzing {len(entity_pairs)} entity pairs using graph methods")
        ...
        for entity_a_id, entity_b_id in entity_pairs:
            ...
            if suggestion:
                ...
                confidence_scores[f"{entity_a_id}|{entity_b_id}"] = suggestion["confidence"]
                
        # Calculate network metrics
        network_metrics = {
            "total_nodes": len(self.nodes),
            "total_edges": len(self.edges),
            "average_connections": sum(len(n.connections) for n in self.nodes.values()) / max(len(self.nodes), 1),
            "relationship_types": len(self.relationship_types),
            "largest_cluster_size": max((len(cluster) for cluster in entity_clusters.values()), default=0)
        }
        ...
        while stack:
            ...
            for connected_id in current_node.connections:
                if (connected_id not in visited and 
                    current_node.connection_strength.get(connected_id, 0) >= self.config["clustering_threshold"]):
    ...
    def _analyze_entity_pair_context(self, entity_a_id: str, entity_b_id: str) -> Optional[Dict[str, Any]]:
        ...
        if direct_relationship:
            ...
            evidence.append(f"Direct {direct_relationship.relationship_type} relationship")
            
        # Shared connections boost confidence
        if shared_connections:
            shared_strength = sum(
                min(node_a.connection_strength.get(conn_id, 0),
            ...
            evidence.append(f"Shared connections with {len(shared_connections)} entities")
        ...
        if context_confidence > 0.1:  # Only return if meaningful context found
            return {
                ...
                "direct_relationship": direct_relationship.relationship_type if direct_relationship else None
            }
            
        return None
        
    def _get_direct_relationship(self, entity_a_id: str, entity_b_id: str) -> Optional[RelationshipEdge]:
