...
from .data_transformer import DataTransformer, load_property_mappings, load_notion_schemas
...
class StagedJSONSyncProcessor(JSONSyncProcessor):
    ...
    def sync_database_transformed(self, database_name: str, stage: int = 1) -> SyncResult:
        ...
        try:
            ...
            if database_name in self.property_mappings:
                ...
                records = self.transformer.transform_database_records(database_name, records, stage)
                if self.verbose:
            ...
            for record in records:
                ...
                if stage < 3:
                    ...
                    if existing_page:
                        ...
                        if self._update_page(existing_page["id"], formatted_properties, result, title_value):
                            # Store page ID for relations
                            self.transformer.set_page_id(database_name, title_value, existing_page["id"])
                    else:
                        # Create new page
                        created_page_id = self._create_page(database_id, formatted_properties, result, title_value)
                        if created_page_id:
                            # Store page ID for relations
                            self.transformer.set_page_id(database_name, title_value, created_page_id)
            ...
            for record in records:
                ...
                if not page_id:
                    ...
                    if existing_page:
                        ...
                        logger.warning(f"No page found for '{title_value}' in {db_name}")
                        continue
                        
                # Get relation updates
                relation_updates = self.transformer.update_relations(record, mapping_config, db_name)
                    ...
                    if not self.dry_run:
                        try:
                            self.notion_updater.client.pages.update(
                                page_id=page_id,
                                properties=relation_updates
                            )
                            updated_count += 1
                            if self.verbose:
                                print(f"   ✅ Updated relations for: {title_value}")
                        except Exception as e:
                            result.errors.append(f"Failed to update relations for '{title_value}': {str(e)}")
    ...
    def _get_title_value(self, record: Dict[str, Any], database_name: str) -> Optional[str]:
    ...
    def _merge_results(self, combined: StagedSyncResult, stage_result: StagedSyncResult, stage: int) -> StagedSyncResult:
