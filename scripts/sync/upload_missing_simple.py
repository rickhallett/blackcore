...
class SimpleMissingRecordsUploader:
    ...
    def format_record_for_notion(
        self, record: Dict[str, Any], db_name: str
    ) -> Dict[str, Any]:
        ...
        if db_name == "Intelligence & Transcripts":
            ...
            if "Source" in record:
                properties["Source"] = {"select": {"name": str(record["Source"])}}
            ...
            if "Type" in record:
                properties["Type"] = {"select": {"name": str(record["Type"])}}
    ...
    def create_notion_page(
        self, database_id: str, properties: Dict[str, Any], title: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "page_id": None, "title": title, "error": str(e)}
    ...
    def upload_all_missing_records(self) -> Dict[str, Any]:
        ...
        for db_name, db_comparison in report["detailed_comparisons"].items():
            ...
            if db_results["records_uploaded"] > 0:
                ...
                upload_results["summary"]["total_records_uploaded"] += db_results[
                    "records_uploaded"
                ]
                logger.info(
                    f"   ✅ Successfully uploaded {db_results['records_uploaded']}/{db_results['records_attempted']} records"
                )
            else:
                upload_results["summary"]["failed_uploads"] += 1
    ...
    def generate_upload_report(self, results: Dict[str, Any]) -> Path:
        ...
        for db_name, db_result in results["databases_processed"].items():
            logger.info(f"\n{db_name}:")
            logger.info(
                f"  Records uploaded: {db_result['records_uploaded']}/{db_result['records_attempted']}"
            )
