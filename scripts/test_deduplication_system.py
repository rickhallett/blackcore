...
def run_comprehensive_test():
    ...
    for db_name, records in test_data.items():
        ...
        result = engine.analyze_database(
            db_name, records, enable_ai=False
        ...
        logger.info(
            f"      High confidence matches: {len(result.high_confidence_matches)}"
        )
        logger.info(
            f"      Medium confidence matches: {len(result.medium_confidence_matches)}"
        )
        ...
        if result.high_confidence_matches:
            ...
            for match in result.high_confidence_matches:
                ...
                name_a = entity_a.get(
                    ...
                    entity_a.get(
                        "Organization Name",
                        entity_a.get("Event / Place Name", "Unknown"),
                    ),
                )
                name_b = entity_b.get(
                    "Full Name",
                    entity_b.get(
                        "Organization Name",
                        entity_b.get("Event / Place Name", "Unknown"),
                    ),
                )
    ...
    total_duplicates = sum(
        r.potential_duplicates for r in comprehensive_results.values()
    )
    ...
    for db_results in comprehensive_results.values():
        for match in (
            db_results.high_confidence_matches + db_results.medium_confidence_matches
        ):
            entity_a_id = (
                f"{match['entity_type']}:{match['entity_a'].get('id', 'unknown')}"
            )
            entity_b_id = (
                f"{match['entity_type']}:{match['entity_b'].get('id', 'unknown')}"
            )
    ...
    if entity_pairs:
        ...
        logger.info(
            f"      Network nodes: {graph_results.network_metrics['total_nodes']}"
        )
        logger.info(
            f"      Network edges: {graph_results.network_metrics['total_edges']}"
        )
        logger.info(f"      Entity clusters: {len(graph_results.entity_clusters)}")
        logger.info(
            f"      Disambiguation suggestions: {len(graph_results.disambiguation_suggestions)}"
        )
        ...
        if graph_results.disambiguation_suggestions:
            ...
            for suggestion in graph_results.disambiguation_suggestions[
                ...
                logger.info(
                    f"         • Confidence: {confidence:.1f}% - {', '.join(evidence[:2])}"
                )
    ...
    if review_task:
        ...
        logger.info(
            f"      Supporting evidence: {len(review_task.supporting_evidence)}"
        )
        logger.info(
            f"      Conflicting evidence: {len(review_task.conflicting_evidence)}"
        )
    ...
    logger.info(
        f"      Total comparisons: {engine_stats['engine_stats']['total_comparisons']}"
    )
    logger.info(
        f"      AI analyses performed: {engine_stats['engine_stats']['ai_analyses_performed']}"
    )
    logger.info(
        f"      Human reviews created: {engine_stats['engine_stats']['human_reviews_created']}"
    )

    logger.info("   🌐 Graph Statistics:")
    logger.info(f"      Nodes processed: {graph_stats['nodes_processed']}")
    logger.info(
        f"      Relationships identified: {graph_stats['relationships_identified']}"
    )
    logger.info(f"      Clusters formed: {graph_stats['clusters_formed']}")

    logger.info("   👥 Review Statistics:")
    logger.info(
        f"      Reviews completed: {review_stats['interface_stats']['reviews_completed']}"
    )
    logger.info(
        f"      Average review time: {review_stats['interface_stats']['average_review_time']:.1f}s"
    )
    ...
    for db_results in comprehensive_results.values():
        for match in (
            ...
            name_a = entity_a.get(
                "Full Name",
                entity_a.get(
                    "Organization Name", entity_a.get("Event / Place Name", "")
                ),
            )
            name_b = entity_b.get(
                "Full Name",
                entity_b.get(
                    "Organization Name", entity_b.get("Event / Place Name", "")
                ),
            )
    ...
    for expected in expected_duplicates:
        for detected in detected_duplicates:
            if (expected[0] in detected and expected[1] in detected) or (
                expected[1] in detected and expected[0] in detected
            ):
                correctly_detected += 1
                logger.info(
                    f"      ✅ Correctly detected: {expected[0]} ↔ {expected[1]}"
                )
                break
        else:
    ...
    logger.info(
        f"   • Processed {total_entities} entities across {len(test_data)} databases"
    )
    logger.info(f"   • Found {total_duplicates} potential duplicates")
    logger.info(
        f"   • Achieved {detection_rate:.1f}% detection accuracy on known duplicates"
    )
    logger.info(
        f"   • Graph analysis identified {graph_stats['relationships_identified']} relationships"
    )
    logger.info("   • System ready for production use with human oversight")

    if detection_rate >= 80:
        logger.info(
            "🎯 PASS: High accuracy deduplication system successfully implemented"
        )
    else:
        logger.info(
            "⚠️  REVIEW: Detection accuracy below 80% - consider tuning parameters"
        )
