...
class NotionToLocalMerger:
    ...
    def clean_notion_record_for_local(self, notion_record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def merge_database(self, db_name: str, missing_records: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.info(f"   📊 Before: {initial_count} | After: {len(current_records)}")
...
def main():
    ...
    try:
        ...
        if results["summary"]["failed_merges"] == 0:
            logger.info("🎉 All missing Notion records successfully merged to local files!")
            logger.info("   Ready to proceed with bidirectional sync verification.")
            return 0
        else:
            logger.warning(f"⚠️  Merge completed with {results['summary']['failed_merges']} failures")
            return 1
