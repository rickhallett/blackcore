...
def test_cli_merge_scenario():
    ...
    if result.success:
        ...
        print("\nVerification:")
        print(f"  Primary entity ID preserved: {result.merged_entity.get('id') == primary.get('id')}")
        print(f"  Primary entity LinkedIn preserved: {result.merged_entity.get('LinkedIn') == 'linkedin.com/in/tonypowell'}")
        print(f"  Secondary entity Organization merged: {'XYZ Corp' in str(result.merged_entity.get('Organization', ''))}")
    else:
        print("❌ Merge failed!")
