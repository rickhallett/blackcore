...
def main():
    ...
    try:
        ...
        logging.info(f"{'='*60}")
        logging.info(f"Total databases processed: {len(processor.STAGE_1_DATABASES + processor.STAGE_2_DATABASES)}")
        ...
        logging.info("\nStage Breakdown:")
        for stage, stats in result.stage_results.items():
            logging.info(f"  Stage {stage}: Created={stats['created']}, Updated={stats['updated']}, Errors={stats['errors']}")
