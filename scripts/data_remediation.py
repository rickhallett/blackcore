...
class DataRemediator:
    ...
    def __init__(self, base_path: Path):
        ...
        self.backup_path = base_path / "backups" / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def fix_people_contacts(self):
        ...
        for transcript in intel_data.get("Intelligence & Transcripts", []):
            ...
            if isinstance(tagged, list):
                for entity in tagged:
                    ...
                    if entity not in ["Survey Manipulation", "Gemini AI", "Data Analysis", "Scrutiny Committee"]:
        ...
        existing_people = {p.get("Full Name", p.get("Person Name", "")) for p in existing_data.get("People & Contacts", [])}
        ...
        logger.info(f"Added {len(new_people)} new people to people_places.json (total: {len(all_people)})")
    ...
    def fix_places_events(self):
        ...
        logger.info(f"Removed {len(organizations)} organizations from places_events.json")
    ...
    def fix_organizations_bodies(self):
        ...
        self.save_json("organizations_bodies.json", {"Organizations & Bodies": deduplicated})
    ...
    def fix_identified_transgressions(self):
        ...
        # Save fixed data
        self.save_json("identified_transgressions.json", {"Identified Transgressions": deduplicated})
        logger.info(f"Fixed identified_transgressions.json with {len(deduplicated)} entries")
    ...
    def fix_documents_evidence(self):
        ...
        documents = data.get("Documents and Evidence", data.get("Documents & Evidence", []))
        ...
        self.save_json("documents_evidence.json", {"Documents & Evidence": standardized})
    ...
    def fix_agendas_epics(self):
        ...
        if phase_groups["Phase 1"]:
            merged_phase1 = self._merge_agendas(
                ...
                "Phase 1: Mobilization & Evidence Gathering"
            )
        ...
        if phase_groups["Phase 2"]:
            merged_phase2 = self._merge_agendas(
                phase_groups["Phase 2"],
                "Phase 2: Pressure & Credibility Attack"
            )
    ...
    def fix_actionable_tasks(self):
        ...
        logger.info(f"Updated {updated_count} task relationships in actionable_tasks.json")
                            ...
                            for rel_value in rel_values:
                                if rel_value and target_file in lookups:
                                    if rel_value not in lookups[target_file]:
                                        ...
                                        if file_key == "intelligence_transcripts" and rel_field == "Tagged Entities":
                                            if rel_value in ["Survey Manipulation", "Gemini AI", "Data Analysis", "Scrutiny Committee"]:
                                                # Check if concept exists in concepts database
                                                if "concepts" in lookups and rel_value not in lookups["concepts"]:
                                                    issues.append(f"Concept '{rel_value}' in {item_title} not found in concepts database")
                                                continue
                                        issues.append(f"Broken relation: {item_title} -> {rel_field} -> '{rel_value}' (not found in {target_file})")
