...
def main():
    ...
    json_path = Path(__file__).parent.parent / "blackcore" / "models" / "json" / "people_places.json"
    ...
    with Progress(
        ...
        result = engine.analyze_database("People & Contacts", people_data, enable_ai=True)
    ...
    console.print(f"  • Medium confidence matches (70-90%): [yellow]{len(result.medium_confidence_matches)}[/yellow]")
    console.print(f"  • Low confidence matches (<70%): [red]{len(result.low_confidence_matches)}[/red]")
    ...
    console.print(f"  • Success rate: {(success_count / len(review_decisions) * 100):.1f}%")
    ...
    console.print(f"  • Primary IDs preserved: {id_preserved_count}/{len(successful_merges)}")
    ...
    console.print(f"  • Final count would be: {len(people_data) - success_count} people")
