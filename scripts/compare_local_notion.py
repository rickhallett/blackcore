...
class LocalNotionComparator:
    ...
    def get_record_title(self, record: Dict[str, Any], db_name: str, is_notion: bool = False) -> str:
        """Extract the title/primary key from a record."""
        # Get title property from config
        title_property = self.notion_config.get(db_name, {}).get("title_property", "Name")
        ...
        # Fallback strategies
        title_candidates = [
            "Full Name", "Name", "Organization Name", "Agenda Title", 
            "Task Name", "Document Name", "Entry Title", "Event / Place Name",
            "Transgression Summary", "Request Name", "Donation ID", "Feature Path"
        ]
    ...
    def compare_database(self, db_name: str, local_records: List[Dict[str, Any]], 
                        notion_records: List[Dict[str, Any]]) -> DatabaseComparison:
        ...
        for title in notion_titles - local_titles:
            missing_from_local.append(RecordComparison(
                title=title,
                in_local=False,
                in_notion=True,
        ...
        for title in local_titles - notion_titles:
            missing_from_notion.append(RecordComparison(
                title=title,
                in_local=True,
                in_notion=False,
    ...
    def compare_record_properties(self, local_record: Dict[str, Any], 
                                 notion_record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def generate_summary_report(self, comparisons: Dict[str, DatabaseComparison]) -> Dict[str, Any]:
    ...
    def save_detailed_report(self, comparisons: Dict[str, DatabaseComparison], 
        ...
        for db_name, comp in comparisons.items():
            detailed_report["detailed_comparisons"][db_name] = {
                ...
                "property_mismatches": [
                    {
                        "title": r.title,
                        "differences": r.property_differences
                    }
...
def main():
    ...
    try:
        ...
        # Final assessment
        total_gaps = summary["totals"]["missing_from_local"] + summary["totals"]["missing_from_notion"]
        
        if total_gaps == 0:
            logger.info("🎉 Perfect synchronization achieved!")
            return 0
        else:
            logger.info(f"⚠️  Found {total_gaps} synchronization gaps that need attention")
            return 1
