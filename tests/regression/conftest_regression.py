...
def bug_reproduction_helpers():
    ...
    class BugReproductionHelpers:
        ...
        @staticmethod
        def reproduce_threshold_boundary_bug(
            confidence, auto_threshold, review_threshold
        ):
    ...
    class RegressionValidationHelpers:
        ...
        def validate_ui_backend_consistency(ui_summary, backend_processing):
            ...
            if ui_summary.get("low_confidence", 0) > 0:
                low_in_review = sum(
                    1
                    for match in backend_processing.get("review_matches", [])
                    if "low" in match.get("id", "").lower()
                )
                if low_in_review != ui_summary["low_confidence"]:
                    errors.append(
                        f"UI shows {ui_summary['low_confidence']} low confidence matches, but only {low_in_review} in review"
                    )
        ...
        def validate_primary_entity_preservation(original_match, final_merge_result):
            ...
            if final_merge_result["id"] != expected_primary_id:
                errors.append(
                    f"Primary entity not preserved: expected {expected_primary_id}, got {final_merge_result['id']}"
                )
        ...
        def validate_threshold_consistency(matches, auto_threshold, review_threshold):
            ...
            for match in matches:
                ...
                if confidence >= auto_threshold and category != "high":
                    errors.append(
                        f"Match {match['id']} with confidence {confidence} should be high, got {category}"
                    )
                elif (
                    ...
                    errors.append(
                        f"Match {match['id']} with confidence {confidence} should be medium, got {category}"
                    )
                elif confidence < review_threshold and category != "low":
                    errors.append(
                        f"Match {match['id']} with confidence {confidence} should be low, got {category}"
                    )
        ...
        def validate_performance_regression(
            current_metrics, baseline_metrics, tolerance=0.2
        ):
                ...
                if metric in baseline_metrics:
                    ...
                    if metric in ["processing_time", "memory_usage"]:
                        ...
                        if current_value > max_allowed:
                            ...
                            errors.append(
                                f"Performance regression in {metric}: {regression_pct:.1f}% worse than baseline"
                            )
...
def mock_merge_executor():
    ...
    def mock_create_proposal(primary_entity, secondary_entity, **kwargs):
        ...
        proposal.proposal_id = (
            f"proposal-{primary_entity['id']}-{secondary_entity['id']}"
        )
    ...
    def mock_execute_merge(proposal, auto_approved=False):
        ...
        merged["_merge_info"] = {
            "merged_from": [
                proposal.primary_entity["id"],
                proposal.secondary_entity["id"],
            ],
