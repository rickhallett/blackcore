...
class TestGoldenPathWorkflows:
    ...
    async def test_complete_deduplication_golden_path(self, mock_cli_with_data):
        ...
        with patch(
            ...
            # Execute the merge
            merge_result = mock_executor.execute_merge(
                mock_proposal, auto_approved=True
            )
    ...
    async def test_manual_review_golden_path(self, mock_cli_with_data):
        ...
        assert (
            len(matches_to_review) == 3
        ), "Should collect all medium + low confidence matches"
    ...
    async def test_configuration_and_analysis_golden_path(self, mock_cli_with_data):
        ...
        async def mock_load_databases():
            return {
                db: data
                for db, data in mock_databases.items()
        ...
        async def mock_analysis_with_progress(*args, **kwargs):
            ...
            mock_result.high_confidence_matches = [
                {"id": f"high-{i}"} for i in range(2)
            ]
            mock_result.medium_confidence_matches = [
                {"id": f"med-{i}"} for i in range(2)
            ]
        ...
        results = await cli.engine.analyze_databases_async(
            databases, progress_callback=mock_progress_callback
        )
    ...
    async def test_error_recovery_golden_path(self, mock_cli_with_data):
        ...
        assert (
            results is not None
        ), f"Analysis should succeed after retry, last error: {last_error}"
        assert "People & Contacts" in results
        ...
        decision = {"match": match, "decision": "merge", "reasoning": "Clear duplicate"}
        ...
        remaining_matches = [
            m for m in session_state["matches"] if m["status"] == "pending"
        ]
    ...
    async def test_data_integrity_golden_path(self, mock_cli_with_data):
        ...
        assert (
            processing_hash == original_hash
        ), "Entities should be unchanged during processing"
        ...
        assert (
            merged_entity["Full Name"] == primary["Full Name"]
    ...
    async def test_performance_golden_path(self, mock_cli_with_data):
        ...
        async def mock_realistic_analysis(*args, **kwargs):
            ...
            mock_result.high_confidence_matches = [
                {"id": f"high-{i}"} for i in range(expected_duplicates // 3)
            ]
            mock_result.medium_confidence_matches = [
                {"id": f"med-{i}"} for i in range(expected_duplicates // 3)
            ]
            mock_result.low_confidence_matches = [
                {"id": f"low-{i}"} for i in range(expected_duplicates // 3)
            ]
        ...
        assert (
            analysis_time < 5.0
        ), f"Analysis too slow: {analysis_time:.2f}s for {dataset_size} entities"

        # Memory usage should be reasonable
        assert (
            memory_usage < 100
        ), f"Memory usage too high: {memory_usage:.1f}MB for {dataset_size} entities"
        ...
        assert (
            db_result.potential_duplicates < dataset_size * 0.1
        )  # Less than 10% duplicates

        # Step 5: Throughput validation
        entities_per_second = (
            dataset_size / analysis_time if analysis_time > 0 else float("inf")
        )
        assert (
            entities_per_second > 100
        ), f"Throughput too low: {entities_per_second:.1f} entities/sec"
