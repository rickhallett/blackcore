...
class TestInputOutputValidation:
    ...
    def test_input_data_validation(self, sample_people_data):
            ...
            if "Email" in data and data["Email"] is not None:
                if not isinstance(data["Email"], (str, list)):
                    ...
                    if not all(isinstance(email, str) for email in data["Email"]):
                        errors.append("invalid_data_type: All emails in list must be strings")
        ...
        for test in invalid_data_tests:
            ...
            assert any(test["validation_error"] in error for error in errors), \
    ...
    def test_list_value_handling(self):
        ...
        for test in list_handling_tests:
            ...
            assert set(list_fields) == set(test["expected_list_fields"]), \
            ...
            for key in entity:
                assert key in normalized, f"Original field '{key}' missing after normalization"
            ...
            for list_field in test["expected_list_fields"]:
                ...
                assert normalized_key in normalized, f"Normalized field '{normalized_key}' not created"
    ...
    def test_conflict_detection_accuracy(self):
        ...
        conflict_test_cases = [
            ...
            },
            {
                ...
                "entity_b": {
                    "id": "b3",
                    "Full Name": "Jane Doe",
                    "Email": ["jane@example.com", "j.doe@company.com"],  # List vs string
            ...
            },
            {
                ...
                "entity_b": {
                    "id": "b4",
                    "Full Name": "Bob Wilson",
                    "Email": ["robert@example.com", "r.wilson@company.com"],  # No overlap
        ...
        for test in conflict_test_cases:
            ...
            assert set(conflicts) == set(test["expected_conflicts"]), \
    ...
    def test_merged_entity_validation(self):
        ...
        for test in merge_validation_tests:
            merged, conflicts = perform_conservative_merge(test["primary"], test["secondary"])
            
            # Validate merged entity structure
            assert merged["id"] == test["expected_merged"]["id"], \
            ...
            assert "merged_from" in merged["_merge_info"], "Merge source tracking missing"
            assert "merge_strategy" in merged["_merge_info"], "Merge strategy not recorded"
    ...
    def test_audit_trail_completeness(self):
        ...
        audit_tests = [
            {
                ...
                "required_fields": ["match_id", "decision", "reasoning", "reviewer", "timestamp"],
            ...
            },
            {
                ...
                "required_fields": ["proposal_id", "primary_entity_id", "secondary_entity_id", "success", "timestamp"],
                "optional_fields": ["merge_strategy", "conflicts_detected", "error_message"]
            },
            {
                ...
                "required_fields": ["database", "total_entities", "matches_found", "timestamp"],
        ...
        for test in audit_tests:
            ...
            assert is_valid, f"Audit validation failed for {test['operation']}: {errors}"
    ...
    def test_data_integrity_preservation(self):
        ...
        def validate_data_integrity(stage, original, processed):
            ...
            if stage == "merge_operation":
                ...
                if not original_ids.issubset(processed_ids):
                    ...
                    integrity_errors.append(f"Missing entity IDs after merge: {missing_ids}")
            else:
                ...
                if original_ids != processed_ids:
                    integrity_errors.append(f"Entity IDs changed: original={original_ids}, processed={processed_ids}")
        ...
        for stage_info in integrity_stages:
            ...
            # Validate integrity
            is_valid, errors = validate_data_integrity(stage, current_data, processed_data)
