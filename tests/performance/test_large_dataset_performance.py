...
class TestLargeDatasetPerformance:
    ...
    def generate_large_dataset(self, size: int) -> List[Dict[str, Any]]:
        ...
        for i in range(size):
            ...
            person = {
                ...
                "Skills": [
                    f"Skill {j}" for j in range(base_index % 5)
                ],  # Variable list size
    ...
    async def test_large_dataset_analysis_performance(self, mock_cli_with_data):
        ...
        for size in dataset_sizes:
            ...
            mock_result.high_confidence_matches = [
                {"id": f"match-{i}"} for i in range(expected_duplicates // 3)
            ]
            ...
            assert (
                results["People & Contacts"].potential_duplicates == expected_duplicates
            )

            # Performance assertions
            assert (
                processing_time < 10.0
            ), f"Processing time too high for {size} entities: {processing_time}s"
            assert (
                memory_usage < 500
            ), f"Memory usage too high for {size} entities: {memory_usage}MB"
        ...
        for i, size in enumerate(dataset_sizes[1:], 1):
            ...
            time_ratio = (
                ...
                / performance_results[prev_size]["processing_time"]
            )
    ...
    async def test_memory_usage_patterns(self, mock_cli_with_data):
        ...
        def record_memory(checkpoint_name):
            ...
            memory_checkpoints.append(
                {"checkpoint": checkpoint_name, "memory_mb": memory_mb}
            )
    ...
    async def test_concurrent_processing_performance(self, mock_cli_with_data):
        ...
        datasets = [self.generate_large_dataset(200) for _ in range(5)]
        ...
        for i in range(len(datasets)):
            ...
            task = mock_cli.engine.analyze_databases_async(
                {f"Dataset_{i}": datasets[i]}
            )
            tasks.append(task)
    ...
    async def test_progress_tracking_performance(self, mock_cli_with_data):
        ...
        for i, update in enumerate(progress_updates[1:], 1):
            ...
            assert (
                update["current"] >= prev_update["current"]
            )  # Should be non-decreasing

        # Performance impact should be minimal
        overhead = (
            (progress_time - no_progress_time) / no_progress_time
            if no_progress_time > 0
            else 0
        )
    ...
    async def test_large_match_review_performance(self, mock_cli_with_data):
        ...
        decisions_per_second = (
            num_matches / decision_time if decision_time > 0 else float("inf")
        )
        merges_per_second = (
            len(approved_merges) / merge_time if merge_time > 0 else float("inf")
        )

        assert (
            decisions_per_second > 50
        ), f"Decision processing too slow: {decisions_per_second:.1f} decisions/sec"
        assert (
            merges_per_second > 10
        ), f"Merge execution too slow: {merges_per_second:.1f} merges/sec"
    ...
    async def test_memory_leak_detection(self, mock_cli_with_data):
        ...
        if len(memory_samples) > 5:
            ...
            assert (
                growth_rate < 0.2
            ), f"Potential memory leak detected: {growth_rate:.2%} growth rate"

        # Total memory growth should be reasonable
        assert (
            memory_growth < 50
        ), f"Excessive memory growth: {memory_growth:.1f}MB after {num_iterations} iterations"
