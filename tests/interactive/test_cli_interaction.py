...
class TestCLIInteraction:
        ...
        menu_scenarios = [
            {
                ...
                "inputs": [
                    ...
                    "y",
                    "y",
                ],  # New analysis, confirm settings, confirm review
            ...
            {"name": "exit_flow", "inputs": ["5"], "expected_calls": ["exit"]},  # Exit
        ]
        ...
        for scenario in menu_scenarios:
            with patch("rich.prompt.Prompt.ask") as mock_prompt:
                with patch("rich.prompt.Confirm.ask") as mock_confirm:
                    ...
                    mock_prompt.side_effect = lambda *args, **kwargs: next(
                        prompt_responses, "1"
                    )
                    ...
                    with patch.object(
                        cli, "_new_analysis", new_callable=AsyncMock
                    ) as mock_new_analysis:
                        with patch.object(
                            cli, "_configure_settings", new_callable=AsyncMock
                        ) as mock_configure:
                            with patch.object(
                                cli, "_view_statistics", new_callable=AsyncMock
                            ) as mock_stats:
                                with patch.object(
                                    cli, "_show_help", new_callable=AsyncMock
                                ) as mock_help:
    ...
    async def test_keyboard_shortcuts_and_navigation(self, mock_cli_with_data):
        ...
        navigation_tests = [
            {"key": "a", "action": "approve", "expected_decision": "merge"},
            {"key": "r", "action": "reject", "expected_decision": "separate"},
            {"key": "d", "action": "defer", "expected_decision": "defer"},
            {
                "key": "s",
                "action": "swap_primary",
                "expected_effect": "primary_swapped",
            },
            {"key": "m", "action": "merge_preview", "expected_effect": "preview_shown"},
            {"key": "n", "action": "next", "expected_effect": "navigation_next"},
            {"key": "p", "action": "previous", "expected_effect": "navigation_prev"},
            {"key": "h", "action": "help", "expected_effect": "help_shown"},
            {"key": "q", "action": "quit", "expected_effect": "quit_prompted"},
        ]
                ...
                if test["input_type"] == "menu_choice":
                    with patch("rich.prompt.Prompt.ask") as mock_prompt:
                        ...
                        try:
                            ...
                            assert (
                                False
                            ), f"Unhandled exception for input '{invalid_input}': {e}"
        ...
        for scenario in help_scenarios:
            ...
            with patch.object(cli.console, "print", side_effect=mock_print):
                ...
                if scenario["context"] == "main_menu":
                    ...
                    # Test review help
                    from blackcore.deduplication.cli.ui_components import (
                        MatchReviewDisplay,
                    )
                ...
                for expected_item in scenario["expected_content"]:
                    assert (
                        expected_item.lower() in output_text
    ...
    async def test_progress_display_and_updates(self, mock_cli_with_data):
            ...
            progress_outputs.append(
                {
                    ...
                    "total": update.total,
                    "percentage": (
                        (update.current / update.total * 100) if update.total > 0 else 0
                    ),
                }
            )
        ...
        ), f"Final progress should be 100%, got {final_progress}%"
    ...
    async def test_error_message_display_and_recovery(self, mock_cli_with_data):
        ...
        for scenario in error_scenarios:
            ...
            with patch.object(cli.console, "print", side_effect=mock_error_display):
                ...
                try:
                    ...
                    if scenario["recovery_action"] == "retry_or_exit":
                        cli.console.print(
                            "Try checking the file path or select a different database."
                        )
                        ...
                        cli.console.print(
                            "Check your API key configuration or disable AI analysis."
                        )
                    elif scenario["recovery_action"] == "retry_or_offline_mode":
                        cli.console.print(
                            "Check your network connection or try offline mode."
                        )
            ...
            for expected_phrase in scenario["expected_message_contains"]:
                assert (
                    expected_phrase in all_error_text
    ...
    async def test_session_state_preservation(self, mock_cli_with_data):
        ...
        session_actions = [
            {
                "action": "load_data",
                "state_changes": {"databases_loaded": True, "entity_count": 100},
            },
            {
                "action": "run_analysis",
                "state_changes": {"analysis_complete": True, "matches_found": 15},
            },
            {
                "action": "review_decisions",
                "state_changes": {"decisions_made": 5, "approved_merges": 3},
            },
        ]
            ...
            if action == "load_data":
                ...
                for i in range(changes["decisions_made"]):
                    decision = {
                        "match": {"id": f"match-{i}"},
                        "decision": (
                            "merge" if i < changes["approved_merges"] else "separate"
                        ),
        ...
        approved_count = sum(
            1 for d in recovered_state["review_decisions"] if d["decision"] == "merge"
        )
