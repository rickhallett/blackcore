...
class TestContinuousSyncWorkflow:
        ...
        def process_daily_transcript(transcript, day):
            ...
            if day == "monday":
                extracted = {
                    ...
                    "tasks": [
                        {"title": "Review requirements", "assignee": "John Smith", "due": "EOD"},
                        {"title": "Complete designs", "assignee": "Jane Doe", "due": "Wednesday"},
                        {"title": "Estimate development time", "assignee": "Bob Wilson"}
                    ]
                }
            elif day == "tuesday":
                extracted = {
                    "people": [
                        ...
                        {"name": "Sarah Chen", "organization": "Beta Industries", "email": "s.chen@beta.com"}
                    ],
                    "organizations": [
                        {"name": "Beta Industries", "type": "client"}
                    ],
                    "tasks": [
                        {"title": "Revise designs with brand colors", "assignee": "Jane Doe"},
                        {"title": "Schedule follow-up", "assignee": "John Smith", "due": "Thursday"}
                    ]
                }
            elif day == "wednesday":
                extracted = {
                    "people": [
                        ...
                        {"name": "Michael Chang", "role": "Senior Developer", "email": "m.chang@acme.com", "phone": "555-0147"},
                        {"name": "Jane Doe"}  # Mentioned for completing task
                    ],
                    "tasks": [
                        {"title": "API integration", "assignee": "Michael Chang"},
                        {"title": "Database schema design", "assignee": "Michael Chang"}
                    ]
                }
            elif day == "thursday":
                extracted = {
                    "people": [
                        {"name": "John Smith"},
                        {"name": "Jane Doe"},
                        {"name": "Sarah Chen", "role": "Project Manager"},  # Role update
                        {"name": "Robert Lee", "role": "CTO", "organization": "Beta Industries", "email": "rlee@beta.com"},
                        {"name": "Elizabeth Chen", "role": "CEO", "organization": "Beta Industries"}
                    ],
                    "tasks": [
                        {"title": "Final designs", "assignee": "Jane Doe", "due": "Friday"},
            ...
            for completion in extracted.get("task_completions", []):
                ...
                if task_title in entity_tracker["tasks"]:
                    ...
                    entity_tracker["tasks"][task_title]["completed_by"] = completion["completed_by"]
                    entity_tracker["tasks"][task_title]["completed_date"] = transcript.date
                    
                    result.updated.append({
                        "type": "task",
                        "title": task_title,
                        "updates": {"status": "completed", "completed_date": transcript.date}
                    })
    ...
    def test_entity_information_updates(self, mock_sync_processor):
        ...
        assert any("Manager" in c["changes"] for c in all_changes if c["entity"] == "David Kim")
    ...
    def test_sync_state_persistence(self, mock_sync_processor, daily_transcripts):
        ...
        assert all(day in new_processor._sync_state for day in ["monday", "tuesday", "wednesday", "thursday", "friday"])
    ...
    def test_incremental_sync_with_deduplication(self, mock_sync_processor):
        ...
        notion_database = {
            "people": [
                {"id": "p1", "Full Name": "Jennifer White", "Email": "jwhite@example.com"},
                {"id": "p2", "Full Name": "Mark Brown", "Organization": "ABC Corp"}
            ],
            "tasks": [
                {"id": "t1", "Title": "Quarterly Review", "Assignee": "p1", "Status": "In Progress"}
            ]
        }
        ...
        new_data_batches = [
            {
                ...
                "entities": {
                    "people": [
                        {"name": "Jennifer White", "phone": "555-0300"},  # Update existing
                        {"name": "J. White", "department": "Finance"},  # Potential duplicate
                        {"name": "Lisa Green", "email": "lgreen@example.com"}  # New person
                    ],
                    "tasks": [
                        {"title": "Budget Planning", "assignee": "Jennifer White"},
                        {"title": "Quarterly Review", "update": "Needs revision"}  # Update existing
                    ]
                }
            },
            {
                ...
                "entities": {
                    "people": [
                        {"name": "Mark Brown", "email": "mbrown@abc.com"},  # Update existing
                        {"name": "M. Brown", "title": "Director"},  # Potential duplicate
                        {"name": "Jennifer W.", "role": "CFO"}  # Another potential duplicate
                    ],
                    "tasks": [
                        {"title": "Q1 Planning", "assignee": "Mark Brown", "due": "2025-01-31"}
                    ]
                }
            }
        ]
        ...
        def incremental_sync_with_dedup(batch, existing_db):
            ...
            for person in batch["entities"]["people"]:
                ...
                for existing in existing_db["people"]:
                    ...
                    elif person["name"].startswith("J.") and "Jennifer" in existing["Full Name"]:
                        confidence = 80
                    ...
                    elif person["name"] in ["M. Brown", "Mark Brown"] and "Mark Brown" in existing["Full Name"]:
                        confidence = 85
            ...
            for task in batch["entities"].get("tasks", []):
                existing_task = next((t for t in existing_db["tasks"] if t["Title"] == task["title"]), None)
                ...
                if existing_task:
                    ...
                    if "assignee" in task:
                        # Find assignee ID
                        assignee = next((p for p in existing_db["people"] if p["Full Name"] == task["assignee"]), None)
                        if assignee:
        ...
        for batch in new_data_batches:
            ...
            if batch["batch_id"] not in mock_sync_processor._sync_state:
                ...
                for update in result["updated"]:
                    entity = next(e for e in notion_database["people"] if e["id"] == update["id"])
        ...
        jennifer = next(p for p in notion_database["people"] if p["Full Name"] == "Jennifer White")
        assert "phone" in jennifer  # Should have phone from batch 1
        
        mark = next(p for p in notion_database["people"] if p["Full Name"] == "Mark Brown") 
    ...
    def test_conflict_resolution_during_sync(self, mock_sync_processor):
                ...
                for field, new_value in source_updates.items():
                    if field in resolved_entity and resolved_entity[field] != new_value:
                        ...
                        if conflict_strategy == "latest_wins":
                            ...
                            if source["timestamp"] > resolved_entity.get("Last Updated", ""):
        ...
        assert latest_wins["resolved_entity"]["Email"] == "alex.thompson@company.com"  # From HR (latest)
        assert latest_wins["resolved_entity"]["Phone"] == "555-0111"  # From Meeting (after CRM)
        assert latest_wins["resolved_entity"]["Department"] == "Sales & Marketing"  # From HR (latest)
        
        # Source priority strategy  
        source_priority = strategy_results["source_priority"]
        assert source_priority["resolved_entity"]["Email"] == "alex.thompson@company.com"  # HR highest priority
        assert source_priority["resolved_entity"]["Department"] == "Sales & Marketing"  # HR highest priority
        
        # Manual review strategy
        manual_review = strategy_results["manual_review"]
        manual_conflicts = [c for c in manual_review["conflicts"] if c["resolution"] == "requires_manual_review"]
