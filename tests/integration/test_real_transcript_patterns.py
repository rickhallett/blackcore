...
class TestRealTranscriptPatterns:
    ...
    def test_messy_transcription_data(self, mock_ai_extractor):
        ...
        def extract_from_messy_transcript(transcript):
            ...
            extracted = {"people": [], "organizations": [], "tasks": [], "issues": []}

            # Process attendees section
            attendees_section = transcript.content.split("ATTENDEES:")[1].split(
                "TRANSCRIPT:"
            )[0]
            ...
            if (
                "Beta Industries" in transcript.content
                or "Beta Industires" in transcript.content
            ):
            ...
            action_section = transcript.content.split("ACTION ITEMS:")[1].split(
                "Meeting adjorned"
            )[0]
        ...
        assert any(
            issue["type"] == "transcription_quality" for issue in extracted["issues"]
        )
    ...
    def test_pronoun_resolution(self, mock_ai_extractor):
        ...
        def resolve_pronouns_in_context(transcript):
            ...
            for i, line in enumerate(lines):
                ...
                for pronoun, gender_hint in pronouns.items():
                    if f" {pronoun} " in line.lower() or line.lower().endswith(
                        f" {pronoun}"
                    ):
                        # Find context
                        context_clue = None
                        ...
                        if context_clue:
                            ...
                            elif "Dr. Michael Park" in context_clue and pronoun in [
                                "he",
                                "him",
                            ]:
                            ...
                            elif "planning department" in context_clue and pronoun in [
                                "they",
                                "them",
                            ]:
        ...
        she_maria = next(
            (
                ...
                if r["pronoun"] == "she" and "concerns" in r["line"]
            ),
            None,
        )
        ...
        he_michael = next(
            (
                r
                for r in resolutions
                if r["pronoun"] == "he" and "consultant" in r["line"]
            ),
            None,
        )
        ...
        she_jane = next(
            (
                r
                for r in resolutions
                if r["pronoun"] == "she" and "liaison" in r["line"]
            ),
            None,
        )
        ...
        they_planning = next(
            (
                r
                for r in resolutions
                if r["pronoun"] == "they" and "review" in r["line"]
            ),
            None,
        )
    ...
    def test_multi_entity_extraction(self, mock_ai_extractor):
                                ...
                                "community_representatives": [
                                    {
                                        ...
                                        "business": "Martinez Cafe",
                                    },
                                    {"name": "Robert Lee", "business": "Lee's Market"},
                                    {
                                        "name": "Susan Kim",
                                        "business": "Kim's Beach Rentals",
                                    },
                                ],
                            }
        ...
        proposal_extraction = next(
            e for e in extractions if e["pattern"] == "submitted a joint proposal"
        )
        assert len(proposal_extraction["entities"]["proposers"]) == 2
        assert any(
            p["name"] == "Thomas Wright"
            for p in proposal_extraction["entities"]["proposers"]
        )
        ...
        partner_extraction = next(
            e for e in extractions if "partnering with" in e["pattern"]
        )
        assert (
            "Beta Industries"
            in partner_extraction["entities"]["primary_partner"]["organization"]
        )
        assert (
            len(partner_extraction["entities"]["primary_partner"]["subsidiaries"]) == 2
        )

        # Check coordination extraction
        coord_extraction = next(
            e for e in extractions if "coordinate with" in e["pattern"]
        )
        ...
        presentation_extraction = next(
            e for e in extractions if "presentations from" in e["pattern"]
        )
        assert len(presentation_extraction["entities"]["expert_presenters"]) == 3
        assert (
            len(presentation_extraction["entities"]["community_representatives"]) == 3
        )
    ...
    def test_temporal_reference_handling(self, mock_ai_extractor):
        ...
        def resolve_temporal_references(transcript):
            ...
            relative_patterns = {
                "last Tuesday": lambda: current_date
                - timedelta(days=(current_weekday + 7 - 1) % 7 or 7),
                "this Friday": lambda: current_date
                + timedelta(days=(4 - current_weekday) % 7),
            ...
            for line in lines:
                ...
                if "third Thursday of each month" in line:
                    ...
                    if current_date.day <= 21:  # Could still be this month
                        next_third_thursday = find_nth_weekday(
                            current_date.year, current_date.month, 3, 3
                        )  # Thursday = 3
                        if next_third_thursday <= current_date:
                            # Move to next month
                            if current_date.month == 12:
                                next_third_thursday = find_nth_weekday(
                                    current_date.year + 1, 1, 3, 3
                                )
                            else:
                                next_third_thursday = find_nth_weekday(
                                    current_date.year, current_date.month + 1, 3, 3
                                )
                    else:
        ...
        # Check specific resolutions
        last_tuesday = next(
            (r for r in resolutions if r["reference"] == "last Tuesday"), None
        )
        assert last_tuesday is not None
        assert (
            last_tuesday["resolved_date"] == "2025-01-07"
        )  # Previous Tuesday from Jan 13 (Monday)

        this_friday = next(
            (r for r in resolutions if r["reference"] == "this Friday"), None
        )
        assert this_friday is not None
        assert this_friday["resolved_date"] == "2025-01-17"  # Next Friday from Jan 13

        thirty_days = next(
            (r for r in resolutions if "30 days" in r["reference"]), None
        )
        assert thirty_days is not None
        assert thirty_days["resolved_date"] == "2025-02-06"  # 30 days from Jan 7

        third_thursday = next(
            (r for r in resolutions if "third Thursday" in r["reference"]), None
        )
        assert third_thursday is not None
        assert (
            third_thursday["resolved_date"] == "2025-02-20"
        )  # Third Thursday of February

        # Verify action items
        assert len(action_items) == 5

        emergency_filing = next(
            (a for a in action_items if "emergency session" in a["task"]), None
        )
        assert emergency_filing is not None
        assert emergency_filing["deadline"] == "2025-01-17"

        env_review = next(
            (a for a in action_items if "environmental review" in a["task"]), None
        )
