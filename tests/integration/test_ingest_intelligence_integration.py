...
class TestIngestIntelligenceIntegration:
            ...
            "transcripts": [
                {
                    ...
                    "tagged_entities": [
                        "Maria Garcia",
                        "Thomas Wright",
                        "Wright Development LLC",
                    ],
                    "key_topics": [
                        "beach hut violations",
    ...
    def test_full_json_package_ingestion(
        self, mock_notion_client, intelligence_package, relation_field_map
    ):
        ...
        mock_notion_client.get_cached_database_id.side_effect = (
            lambda name: database_ids.get(name)
        )
        ...
        def ingest_package_two_pass(package):
            ...
            for person in package["people"]:
                ...
                mock_find_or_create_page(
                    "People & Contacts", person["name"], properties
                )
            ...
            for org in package["organizations"]:
                ...
                mock_find_or_create_page(
                    "Organizations & Bodies", org["name"], properties
                )
            ...
            for transgression in package["transgressions"]:
                ...
                mock_find_or_create_page(
                    "Identified Transgressions", transgression["title"], properties
                )
            ...
            for doc in package["documents"]:
                ...
                mock_find_or_create_page(
                    "Documents & Evidence", doc["title"], properties
                )
            ...
            for transcript in package["transcripts"]:
                ...
                mock_find_or_create_page(
                    "Intelligence & Transcripts", transcript["title"], properties
                )
    ...
    def test_relation_preservation_during_sync(
        self, mock_notion_client, intelligence_package
    ):
            ...
            for key, entity in entities.items():
                ...
                if db_name == "People & Contacts":
                    ...
                    if "Linked Transgressions" in props:
                        ...
                        relationships_created["person_to_transgressions"][
                            entity["id"]
                        ] = transgression_ids
                    ...
                    if "Key People" in props:
                        ...
                        relationships_created["org_to_people"][
                            entity["id"]
                        ] = people_ids
                    ...
                    if "Assignee" in props:
                        ...
                        relationships_created["task_to_person"][
                            entity["id"]
                        ] = assignee_ids
                    ...
                    if "Actionable Tasks" in props:
                        ...
                        relationships_created["agenda_to_tasks"][
                            entity["id"]
                        ] = task_ids
    ...
    def test_notion_api_integration(self, mock_notion_client, intelligence_package):
        ...
        mock_responses = {
            ...
            "query_database": {"results": [], "has_more": False},
        }
        ...
        mock_notion_client.create_page.side_effect = (
            lambda *args, **kwargs: mock_api_call_with_rate_limit("create_page")
        )
        mock_notion_client.update_page_properties.side_effect = (
            lambda *args, **kwargs: mock_api_call_with_rate_limit("update_page")
        )
    ...
    def test_error_recovery_during_ingestion(
        self, mock_notion_client, intelligence_package
    ):
        ...
        def ingest_with_error_handling(package, state):
            ...
            for entity_type, entities in all_entities:
                for entity in entities:
                    entity_key = (
                        f"{entity_type}:{entity.get('name') or entity.get('title')}"
                    )
                    ...
                    try:
                        ...
                        is_recoverable = any(
                            isinstance(e, type(scenario["error"]))
                            and scenario["recoverable"]
                        ...
                        if is_recoverable and state["retry_attempts"][entity_key] < 3:
                            # Retry later
                            print(
                                f"Will retry {entity_key} (attempt {state['retry_attempts'][entity_key]})"
                            )
                        else:
                            # Give up
        ...
        assert all(
            entity in resumed_state["entities_processed"]
            for entity in final_state["entities_processed"]
        )

    def test_complex_relationship_cycles(
        self, mock_notion_client, intelligence_package
    ):
        ...
        def resolve_circular_relationships(entities):
            ...
            for person in entities["people"]:
                ...
                all_entities[key] = {"type": "person", "data": person, "relations": {}}
            ...
            for person in entities["people"]:
                ...
                if "collaborates_with" in person:
                    for collaborator in person["collaborates_with"]:
                        ...
                        all_entities[person_key]["relations"]["collaborates_with"] = (
                            all_entities[person_key]["relations"].get(
                                "collaborates_with", []
                            )
                            + [collab_key]
                        )
            ...
            while len(resolved) < len(all_entities):
                ...
                if not ready:
                    ...
                    ready = [
                        min(remaining, key=lambda k: len(dependencies[k] - resolved))
                    ]
