...
class TestEntityResolutionDuringIngestion:
        ...
        return {
            ...
            "organizations": [
                ...
                },
                {
                    ...
                    "Also Known As": ["Planning Dept", "City Planning", "Planning Department"],
                    "Type": "Government"
                }
            ]
        }
    ...
    def test_fuzzy_name_matching(self, existing_entities_db):
        ...
        def match_person_fuzzy(extracted_name: str, context: str, existing_people: List[Dict]) -> List[Dict]:
            ...
            for person in existing_people:
                ...
                if confidence == 0 and context:
                    ...
                    if person.get("Organization"):
                        ...
                        if any(org_part in context.lower() for org_part in org_lower.split()):
                            # Partial name match + org match
                            name_parts = name_lower.split()
                            full_parts = full_name.split()
                            matching_parts = sum(1 for part in name_parts if any(part in fp for fp in full_parts))
        ...
        for entity in extracted_entities:
            matches = match_person_fuzzy(entity["name"], entity["context"], existing_entities_db["people"])
        ...
        j_smith_results = [r for r in resolution_results if "J. Smith" in r["extracted"] or "J Smith" in r["extracted"]]
        ...
        smith_john = next(r for r in resolution_results if r["extracted"] == "Smith, John")
        ...
        bob_result = next(r for r in resolution_results if r["extracted"] == "Bob Johnson")
        ...
        e_chen_result = next(r for r in resolution_results if r["extracted"] == "E. Chen")
    ...
    def test_contextual_entity_matching(self, existing_entities_db):
        ...
        def match_with_context(entity: Dict, existing_people: List[Dict], existing_orgs: List[Dict]) -> Dict:
                ...
                for person in existing_people:
                    ...
                    if role in person_title:
                        ...
                        if entity.get("partial_name"):
                            if entity["partial_name"].lower() in person["Full Name"].lower():
                                confidence = 95
                        matches.append({
                            "person": person,
                            "confidence": confidence,
            ...
            # Handle single name with organization context
                ...
                for person in existing_people:
                    if person.get("Organization"):
                        ...
                        if person_org == mentioned_org or any(aka.lower() == person_org for aka in ["Acme Corp", "Acme Corp.", "ACME"]):
        ...
        results = []
        for entity in extracted_entities:
            result = match_with_context(entity, existing_entities_db["people"], existing_entities_db["organizations"])
    ...
    def test_ambiguous_entity_handling(self, existing_entities_db):
        ...
        ambiguous_entities = [
            ...
            {
                "name": "Jane",
                "context": "Jane will coordinate",
                "no_dept_info": True
            }
        ]
        
        def find_all_possible_matches(entity: Dict, existing_people: List[Dict]) -> List[Dict]:
        ...
        for entity in ambiguous_entities:
            all_matches = find_all_possible_matches(entity, existing_entities_db["people"])
            
            result = {
                "extracted": entity["name"],
                "possible_matches": len(all_matches),
                "matches": all_matches,
                "action_required": "manual_disambiguation" if len(all_matches) > 1 else "create_new"
            }
            ...
            if len(all_matches) > 1:
                ...
                if max(confidence_scores) >= 80 and confidence_scores[0] - confidence_scores[1] >= 20:
                    # Clear winner
                    result["recommended_match"] = all_matches[0]["person"]["Full Name"]
                    result["disambiguation_reason"] = "High confidence with significant gap"
                else:
                    # Too close to call
                    result["recommended_action"] = "present_options_to_user"
                    result["disambiguation_reason"] = "Multiple similar confidence matches"
        ...
        assert smith_result["possible_matches"] >= 3  # John Smith, Jane Smith, Jane Doe-Smith
        ...
        j_smith_result = next(r for r in ambiguous_results if r["extracted"] == "J. Smith")
        assert j_smith_result["possible_matches"] >= 2
        assert any("John Smith" in m["person"]["Full Name"] for m in j_smith_result["matches"])
        assert any("Jane Smith" in m["person"]["Full Name"] for m in j_smith_result["matches"])
        
        # "Jane" should match multiple Janes
        jane_result = next(r for r in ambiguous_results if r["extracted"] == "Jane")
        assert jane_result["possible_matches"] >= 2  # Jane Doe, Jane Smith, maybe Jane Doe-Smith
    ...
    def test_organization_name_resolution(self, existing_entities_db):
        ...
        extracted_orgs = [
            ...
            {"name": "City Planning Department", "context": "City Planning Department review"},
            {"name": "Planning", "context": "Sent to Planning for review"},
            {"name": "Johnson Associates", "context": "Johnson Associates proposal"},  # Missing &
        ...
        def match_organization(extracted_org: str, existing_orgs: List[Dict]) -> Dict:
            ...
            for org in existing_orgs:
                ...
                if confidence == 0:
                    ...
                    if extracted_lower in org_name_lower or org_name_lower in extracted_lower:
                        confidence = 80
                        reasoning.append("Partial name match")
                        
                    # Check word overlap
                    elif any(word in org_name_lower.split() for word in extracted_lower.split() if len(word) > 3):
                        confidence = 70
        ...
        for ext_org in extracted_orgs:
            match = match_organization(ext_org["name"], existing_entities_db["organizations"])
        ...
        acme_corp_result = next(r for r in org_results if r["extracted"] == "Acme Corp.")
        assert acme_corp_result["confidence"] >= 95
        
        # Planning variations should match City Planning Department
        planning_results = [r for r in org_results if "planning" in r["extracted"].lower()]
        assert all(r["matched_to"] == "City Planning Department" for r in planning_results)
        
        # "Planning Dept" should match via alias
        planning_dept_result = next(r for r in org_results if r["extracted"] == "Planning Dept")
        assert planning_dept_result["confidence"] >= 95
        
        # "Johnson Associates" (missing &) should still match
        johnson_result = next(r for r in org_results if r["extracted"] == "Johnson Associates")
    ...
    def test_cross_reference_validation(self, existing_entities_db):
        ...
        def validate_with_relationships(entities: Dict, existing_data: Dict) -> List[Dict]:
            ...
            for person in entities["people"]:
                ...
                for existing_person in existing_data["people"]:
                    ...
                    # Name matching
                    if person["name"] == "J. Smith" and "John Smith" in existing_person["Full Name"]:
                        confidence += 40
                        validation_points.append("Name pattern match")
                        
                    elif person["name"] == "Jane D." and existing_person["Full Name"].startswith("Jane"):
                        confidence += 40
                        validation_points.append("First name match")
                        
                    elif person["name"] == "Bob" and existing_person.get("Nickname") == "Bob":
                        confidence += 50
                ...
                if person_name in people_matches:
                    ...
                    if relationship.get("reports_to"):
                        ...
                        if reports_to_name in people_matches:
                            ...
                            # Check organizations
                            if reporter.get("Organization") == manager.get("Organization"):
                                match["confidence"] += 10
                                match["validation_points"].append("Reporting relationship validates org match")
                    ...
                    if relationship.get("works_with"):
                        ...
                        if colleague_name in people_matches:
                            ...
                            if (match["person"].get("Organization") == colleague.get("Organization") or
                                (match["person"].get("Department") and colleague.get("Department"))):
                                match["confidence"] += 5
                                match["validation_points"].append("Colleague relationship validates match")
        ...
        # Run validation
        validated_results = validate_with_relationships(extracted_with_relations, existing_entities_db)
        
        # Verify relationship validation
        
        # J. Smith should have high confidence due to multiple validations
        j_smith = next(r for r in validated_results if r["extracted_name"] == "J. Smith")
