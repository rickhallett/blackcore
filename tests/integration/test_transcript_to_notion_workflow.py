...
class TestTranscriptToNotionWorkflow:
    ...
    def test_new_transcript_with_existing_entities(self, mock_transcript_processor, mock_dedupe_engine, sample_transcript):
        """Test processing a transcript that mentions existing entities in Notion."""
        
        # Mock existing entities in Notion
        existing_entities = {
            "people": [
                {"id": "person-1", "Full Name": "John Smith", "Email": "john@acme.com", "Organization": "Acme Corp"},
                {"id": "person-2", "Full Name": "Jane Doe", "Email": "jane@planning.gov", "Department": "Planning"}
            ],
            "organizations": [
        ...
        extracted_entities = {
            "people": [
                {"name": "Mayor Johnson", "role": "Mayor", "mentioned_in": "Meeting with Mayor Johnson"},
                {"name": "John Smith", "organization": "Acme Corp", "mentioned_in": "John Smith from Acme Corp"},
                {"name": "Jane Doe", "department": "planning department", "mentioned_in": "Jane Doe from the planning department"}
            ],
            "tasks": [
                {"title": "Review survey data", "assignee": "John Smith", "due_date": "Friday"},
                {"title": "Prepare planning report", "assignee": "Jane Doe"},
                {"title": "Schedule follow-up meeting", "assignee": "Mayor Johnson", "timeline": "next week"}
            ],
            "organizations": [
                ...
                "medium_confidence_matches": [
                    {
                        ...
                        "entity_b": {"name": "Jane Doe", "department": "planning department"},
        ...
        def process_with_deduplication(transcript, extracted_entities, existing_entities):
                ...
                for existing in existing_entities["people"]:
                    if person["name"].lower() in existing["Full Name"].lower():
                        matches.append({
                            "new": person,
                            "existing": existing,
                            "confidence": 95.0 if person.get("organization") else 85.0
                        })
            ...
            new_people = [p for p in extracted_entities["people"] 
        ...
        result = process_with_deduplication(sample_transcript, extracted_entities, existing_entities)
        ...
        assert "review_required" not in john_update or not john_update["review_required"]
        ...
        review_task = next(t for t in result.created if t["data"]["title"] == "Review survey data")
        assert review_task["data"]["assignee_id"] == "person-1"  # John Smith's ID
        
        planning_task = next(t for t in result.created if t["data"]["title"] == "Prepare planning report")
        assert planning_task["data"]["assignee_id"] == "person-2"  # Jane Doe's ID
        
        meeting_task = next(t for t in result.created if t["data"]["title"] == "Schedule follow-up meeting")
        assert meeting_task["data"]["assignee_id"] == "new-person-Mayor Johnson"  # New person
        
    def test_transcript_creating_linked_entities(self, mock_transcript_processor, sample_transcript):
        ...
        def create_linked_entities(entities):
            ...
            result.created.append({
                ...
                "data": {
                    **entities["events"][0],
                    "attendee_ids": [person_id]  # Mayor would need to be resolved separately
                }
            })
    ...
    def test_incremental_transcript_processing(self, mock_transcript_processor):
            ...
            if "Initial Planning" in transcript.title:
                ...
                entities = {
                    "people": [
                        ...
                        {
                            "name": "Alice Wong",
                            "role": "Senior Developer"  # New role
                        }
                    ],
                    "tasks": [{
                        "title": "Complete project",
        ...
        assert processing_state["entities"]["people"]["John Smith"]["role"] == "Technical Lead"
        assert processing_state["entities"]["people"]["John Smith"]["email"] == "john.smith@acme.com"
        
    def test_transcript_duplicate_prevention(self, mock_transcript_processor, sample_transcript):
        ...
        def process_transcript_with_duplicate_check(transcript):
            ...
            transcript_hash = f"{transcript.title}_{transcript.date}_{len(transcript.content)}"
            ...
            entities = {
                ...
                "tasks": [
                    {"title": "Review survey data", "assignee": "John Smith"}
                ]
            }
    ...
    def test_ai_extraction_to_deduplication_pipeline(self, mock_transcript_processor, mock_dedupe_engine):
        ...
        ai_extracted = {
            "people": [
                {"name": "John Smith", "organization": "Acme Corp", "context": "presenting Q4 results"},
                {"name": "J. Smith", "organization": "Acme", "context": "will handle follow-ups"},
                {"name": "Jane", "department": "Planning Dept", "context": "from Planning Dept"},
                {"name": "Jane D.", "department": "city planning", "context": "from city planning"},
                {"name": "Bob Wilson", "role": "CTO", "context": "our new CTO"},
                {"name": "Robert Wilson", "role": "Chief Technology Officer", "context": "Chief Technology Officer"}
            ]
        }
