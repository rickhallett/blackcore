...
class TestCLIConsistency:
    ...
    @pytest.mark.asyncio
    async def test_summary_vs_review_consistency(
        self, mock_cli_with_data, workflow_validator
    ):
        ...
        ), f"Summary consistency errors: {consistency_errors}"
        ...
        expected_total = (
            summary_counts["high_confidence"]
            + summary_counts["medium_confidence"]
            + summary_counts["low_confidence"]
        )
        actual_total = len(matches_to_review)

        assert (
            expected_total == actual_total
        ), f"Expected {expected_total} matches for review, got {actual_total}"

    @pytest.mark.asyncio
    async def test_count_validation_edge_cases(
        self, mock_cli_with_data, workflow_validator
    ):
        ...
        for case in edge_cases:
            ...
            assert (
                len(matches_to_review) == case["expected_review_count"]
            ...
            assert (
                len(review_errors) == 0
            ), f"Edge case '{case['name']}' failed: {review_errors}"
    ...
    async def test_progress_tracking_accuracy(self, mock_cli_with_data):
        ...
        # Create test data
        test_entities = [
            {"id": f"entity-{i}", "Full Name": f"Person {i}"} for i in range(5)
        ]
    ...
    async def test_configuration_consistency(self, mock_cli_with_data):
        ...
        for test_config in config_tests:
            ...
            if test_config["expected_review_includes_high"]:
                assert (
                    high_confidence_included
                ), f"High confidence matches should be included with config: {test_config['config']}"
            else:
                assert (
                    not high_confidence_included
                ), f"High confidence matches should NOT be included with config: {test_config['config']}"

            # Always expect medium and low confidence
            medium_included = any(
                match.get("id") == "medium-1" for match in matches_to_review
            )
            low_included = any(
                match.get("id") == "low-1" for match in matches_to_review
            )

            assert (
                medium_included
            ), "Medium confidence matches should always be included"
