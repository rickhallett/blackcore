    ...
    async def run_workflow(self, cli_instance, user_actions: List[Dict[str, Any]]):
        ...
        for action in user_actions:
            ...
            if action_type == "menu_choice":
                ...
                results.append({"action": "menu_choice", "choice": choice, "result": result})
                
            elif action_type == "config_setting":
                # Simulate configuration setting
                setting = action.get("setting")
                value = action.get("value")
                result = await self._simulate_config_setting(cli_instance, setting, value)
                results.append({"action": "config_setting", "setting": setting, "value": value, "result": result})
                ...
                results.append({"action": "review_decision", "decision": decision, "result": result})
                
            elif action_type == "confirm":
                # Simulate yes/no confirmation
                confirm = action.get("value")
                result = await self._simulate_confirmation(cli_instance, confirm)
                results.append({"action": "confirm", "value": confirm, "result": result})
    ...
    async def _simulate_config_setting(self, cli_instance, setting: str, value: Any):
        ...
        if hasattr(cli_instance, 'config_wizard'):
            if setting == "ai_enabled":
                cli_instance.config_wizard.config = cli_instance.config_wizard.config or {}
...
class WorkflowValidator:
    ...
    @staticmethod
    def validate_summary_consistency(summary_counts: Dict[str, int], 
                                   actual_matches: Dict[str, List]) -> List[str]:
        """Validate that summary counts match actual match data."""
        errors = []
        
        # Check high confidence matches
        if summary_counts.get("high_confidence", 0) != len(actual_matches.get("high_confidence", [])):
            errors.append(
        ...
        if summary_counts.get("medium_confidence", 0) != len(actual_matches.get("medium_confidence", [])):
            errors.append(
        ...
        if summary_counts.get("low_confidence", 0) != len(actual_matches.get("low_confidence", [])):
            errors.append(
    ...
    @staticmethod
    def validate_review_availability(summary_counts: Dict[str, int],
                                   matches_for_review: List) -> List[str]:
