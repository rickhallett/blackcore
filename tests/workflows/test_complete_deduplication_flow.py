...
class TestCompleteDeduplicationFlow:
    ...
    @pytest.mark.asyncio
    async def test_happy_path_full_workflow(self, mock_cli_with_data, cli_runner, workflow_validator):
        ...
        workflow_actions = [
            ...
            {"type": "config_setting", "setting": "databases", "value": ["People & Contacts"]},
            {"type": "confirm", "value": True},  # Confirm review matches
            {"type": "review_decision", "decision": "a"},  # Approve first match
            {"type": "review_decision", "decision": "s"},  # Swap primary for second match
        ...
        cli.engine.analyze_databases_async = AsyncMock(return_value={"People & Contacts": mock_result})
        ...
        assert summary_counts["high_confidence"] + summary_counts["medium_confidence"] + summary_counts["low_confidence"] == len(matches_to_review)
        ...
        for interruption_point in interruption_points:
            ...
            if interruption_point == "analysis":
                ...
                with pytest.raises(asyncio.CancelledError):
                    cli.engine.analyze_databases_async.side_effect = asyncio.CancelledError()
                    databases = await cli._load_databases()
                ...
                assert len(cli.current_results["People & Contacts"].high_confidence_matches) == 1
    ...
    async def test_workflow_error_handling(self, mock_cli_with_data):
        ...
        error_scenarios = [
            {"type": "database_load_error", "exception": FileNotFoundError("Database not found")},
        ...
        for scenario in error_scenarios:
            ...
            if scenario["type"] == "database_load_error":
                ...
                with patch.object(cli, '_load_databases', side_effect=scenario["exception"]):
                    with pytest.raises(FileNotFoundError):
                        await cli._load_databases()
                        
            elif scenario["type"] == "analysis_error":
                # Test analysis error
                cli.engine.analyze_databases_async = AsyncMock(side_effect=scenario["exception"])
